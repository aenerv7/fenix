...
- Every shield panel banner state is text-only and left-aligned: the "is on guard", not-secure, and
  protections-off banners must not render the upstream Mozilla fox illustrations
  (`kit_head_protection_blocker_banner`, `protection_panel_not_secure`,
  `protection_panel_not_protected`), all three of which are standalone vectors rather than part of a
  plate. The protected banner's purple-to-orange gradient plate is drawn by Compose and is kept.
