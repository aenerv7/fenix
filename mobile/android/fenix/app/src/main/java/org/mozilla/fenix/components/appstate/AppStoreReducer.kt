...
internal object AppStoreReducer {
    ...
    fun reduce(state: AppState, action: AppAction): AppState =
        when (action) {
            ...
            is AppAction.AddNonFatalCrash -> state.copy(nonFatalCrashes = state.nonFatalCrashes + action.crash)
            is AppAction.RemoveNonFatalCrash -> state.copy(nonFatalCrashes = state.nonFatalCrashes - action.crash)
            is AppAction.RemoveAllNonFatalCrashes -> state.copy(nonFatalCrashes = emptyList())
            ...
            is AppAction.RemoveRecentTab -> {
                state.copy(recentTabs = state.recentTabs.filterOutTab(action.recentTab))
            }
            is AppAction.RecentSyncedTabStateChange -> {
                state.copy(recentSyncedTabState = action.state)
            }
            ...
            is AppAction.RecentHistoryChange -> state.copy(recentHistory = action.recentHistory)
            is AppAction.RemoveRecentHistoryHighlight ->
                state.copy(
                    recentHistory =
                        state.recentHistory.filterNot {
            ...
            is AppAction.RemoveRecentSyncedTab ->
                state.copy(
                    recentSyncedTabState =
                        when (state.recentSyncedTabState) {
                            is RecentSyncedTabState.Success ->
                                RecentSyncedTabState.Success(state.recentSyncedTabState.tabs - action.syncedTab)
            ...
            is AppAction.WallpaperAction.UpdateCurrentWallpaper ->
                state.copy(wallpaperState = state.wallpaperState.copy(currentWallpaper = action.wallpaper))
            is AppAction.WallpaperAction.UpdateAvailableWallpapers ->
                state.copy(wallpaperState = state.wallpaperState.copy(availableWallpapers = action.wallpapers))
            is AppAction.WallpaperAction.UpdateWallpaperDownloadState -> {
                val wallpapers =
                    state.wallpaperState.availableWallpapers.map {
            ...
            is AppAction.UpdateStandardSnackbarErrorAction ->
                state.copy(standardSnackbarError = action.standardSnackbarError)

            is AppAction.TabStripAction.UpdateLastTabClosed -> state.copy(wasLastTabClosedPrivate = action.private)

            is AppAction.TranslationsAction.TranslationStarted ->
                state.copy(snackbarState = SnackbarState.TranslationInProgress(sessionId = action.sessionId))
            ...
            is AppAction.BookmarkAction.BookmarkDeleted ->
                state.copy(snackbarState = SnackbarState.BookmarkDeleted(title = action.title))

            is AppAction.BookmarkAction.BookmarkOperationResultReported ->
                state.copy(snackbarState = SnackbarState.BookmarkOperationResultReported(action.globalResultReport))

            is AppAction.DeleteAndQuitStarted -> {
                state.copy(snackbarState = SnackbarState.DeletingBrowserDataInProgress)
            }

            is AppAction.CurrentTabClosed ->
                state.copy(snackbarState = SnackbarState.CurrentTabClosed(action.isPrivate))

            is AppAction.URLCopiedToClipboard -> state.copy(snackbarState = SnackbarState.URLCopiedToClipboard)
            ...
            is AppAction.UpdateDefaultBrowserStatus -> state.copy(isDefaultBrowser = action.isDefault)

            is AppAction.UserAccountAuthenticated -> state.copy(snackbarState = SnackbarState.UserAccountAuthenticated)

            is VoiceSearchAction ->
                state.copy(voiceSearchState = VoiceSearchReducer.reduce(state.voiceSearchState, action))
            ...
            is AppAction.CrashActionWrapper -> {
                val newSnackbarState =
                    if (action.inner is CrashAction.ReportTapped) {
            ...
            is AppAction.ContentRecommendationsAction ->
                ContentRecommendationsReducer.reduce(
                    state = state,
                    action = action,
                )

            is AppAction.WebCompatAction -> WebCompatReducer.reduce(state = state, action = action)
            is AppAction.SetupChecklistAction ->
                SetupChecklistReducer.reduce(
                    state = state,
                    action = action,
                )

            is AppAction.DownloadAction.DownloadInProgress ->
                state.copy(snackbarState = SnackbarState.DownloadInProgress(action.downloadId))

            is AppAction.DownloadAction.DownloadFailed ->
                state.copy(snackbarState = SnackbarState.DownloadFailed(action.fileName))

            is AppAction.DownloadAction.DownloadCompleted ->
                state.copy(
                    snackbarState = SnackbarState.DownloadCompleted(action.downloadState),
                    supportedMenuNotifications =
                        state.supportedMenuNotifications + SupportedMenuNotifications.Downloads,
                )

            is AppAction.DownloadAction.CannotOpenFile ->
                state.copy(snackbarState = SnackbarState.CannotOpenFileError(action.downloadState))
            ...
            is AppAction.MenuNotification.AddMenuNotification ->
                state.copy(supportedMenuNotifications = state.supportedMenuNotifications + action.notification)

            is AppAction.MenuNotification.RemoveMenuNotification ->
                state.copy(supportedMenuNotifications = state.supportedMenuNotifications - action.notification)
